...
template<typename DOCIDTYPE, typename idtype, typename dist_t>
class MultiVectorSearchStopCondition : public BaseSearchStopCondition<idtype, dist_t> {
 ...
 public:
    ...
    void add_point_to_result(idtype label, const void *datapoint, dist_t dist) override {
    ...
    void remove_point_from_result(idtype label, const void *datapoint, dist_t dist) override {
        DOCIDTYPE doc_id = space_.get_doc_id(datapoint);
        doc_counter_[doc_id] -= 1;
        if (doc_counter_[doc_id] == 0) {
            curr_num_docs_ -= 1;
    ...
    void filter_results(std::vector<std::pair<dist_t, idtype >> &candidates) override {
...
class EpsilonSearchStopCondition : public BaseSearchStopCondition<idtype, dist_t> {
    float epsilon_;
